...
list_surveys = list()
for (h in 1:b) {
  list_surveys[[h]] = readRDS(paste0('~/GitHub/NSUM-Simulations/data/initial data/survey_',h,'.rds'))
}
...
################################################################################
file_name = str_c(paste0("Simulation_transmission_notdisjoint_",hpdistrib,
                         "_",net,".csv"))
